 ...
 *
 * This file is modified by Tähvend Uustalu to show correct checker messages
 * for EIO CMS.
 */
...
// On CMS, checker must always return 0
#ifndef OK_EXIT_CODE
#   ifdef CONTESTER
...
#   else
// fail is essentially a checker crash,
...
NORETURN void InStream::quit(TResult result, const char *msg) {
    ...
    if (result == _ok) {
      std::cout << 1 << std::endl;
    } else if (first_test_passed) {
      std::cout << 0.25 << std::endl;
    } else {
      std::cout << 0 << std::endl;
    }

#ifdef HIDE_CHECKER_OUTPUT
    switch (result) {
    case _ok:
    ...
    case _wa:
      ...
      break;
    case _points:
      std::cerr << __testlib_points * 100 << "% points" << std::endl;
      break;
    case _fail:
      std::cerr << "Got better results than the jury. Please notify them using the contest system." << std::endl;
      break;
    case _pe:
      // in this case, we can be friendly
      std::cerr << msg << std::endl;
      break;
    default:
      // unsupported exit code, problemsetters fault if this reveals something
      std::cerr << msg << std::endl;
    }
#else
    std::cerr << msg << std::endl;
...
void registerTestlibCmd(int argc, char *argv[]) {
    ...
    // EIO CMS: our order of arguments is inf ans ouf
    inf.init(argv[1], _input);
